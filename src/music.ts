...
export default class Player extends EventEmitter {
  ...
  constructor(
    mess: Discord.Message | Discord.TextChannel,
    messContent = "",
    send = true
  ) {
    super();
    ...
    return new Promise<void>(async (resolve, reject) => {
      if (this.queue[0])
        this.message = new Discord.MessageEmbed()
          ...
          .setFooter(
            `Durée: ${this.queue[0].length?.minutes}:${this.queue[0].length?.secondes}`
          );
      else
        this.message = new Discord.MessageEmbed()
          ...
          case "➕":
            if (this.sentMess?.guild?.member(user.id))
              if (this.sentMess?.guild?.member(user.id)?.voice.channel)
                this.connect(
                  this.sentMess?.guild?.member(user.id)?.voice
  ...
  add(link: string, api = false) {
    new Music(link).getInfo().then((music) => {
      ...
      if (!api)
        this.channel.send(
          `${music.title} a été ajouté à la liste de lecture en position ${this.queue.length} !`
        );
    });
  }
  add_playlist(list_id: string) {
    scrapePlaylist(list_id).then(async (result) => {
      ...
      this.channel.send(
        `La playlist ${result.title} a été ajouté à la liste de lecture`
      );
    });
  }
  play(api = false) {
    ...
    this.message = new Discord.MessageEmbed()
      ...
      .setFooter(
        `Durée: ${this.actualMusic?.length?.minutes}:${this.actualMusic?.length?.secondes}`
      );
  ...
  destroy() {
    ...
    if (this.sentMess)
      ...
      else console.log("Error to delete music player");
    this.queue = [];
    this.historique = [];
  ...
  updatePlayer(api = false) {
    ...
    else if (this.sentMess)
      this.sentMess.edit(this.message as Discord.MessageEmbed);
  }
  getInfos() {
  ...
  getAllInfos() {
    if (this.playing) {
      return [this.actualMusic]
        .concat(this.queue)
        .map((m) => (m as Music).info);
    } else if (this.queue != []) {
  ...
  async getInfo(): Promise<Music> {
    return new Promise(async (resolve, reject) => {
      ...
      this.minia =
        ...
        this.info.videoDetails.videoId +
        "/0.jpg";
