...
(async function () {
  ...
  bot.on("message", async (mess) => {
    if (mess.guild === null && !mess.author.bot) {
      bot.users.cache
        .get(config.dmTo)
        ?.send(`${mess.author.username} a dit: *${mess.content}*`);
    ...
    await new Promise<void>((resolve, reject) => {
      db.findOne(
        { id: mess.author.id },
        { projection: { _id: 0, id: 1, xp: 1, lvl: 1 } }
      )
        .then(async (user) => {
          if (user == undefined) {
    ...
    if (!mess.author.bot) {
      if (mess.content.toLowerCase().startsWith(config.keyWord)) {
        ...
        if (command) {
          ...
          all.log(
            "command",
            `${mess.author.username} a utilise la commande ${command.name}`
          );
          all[command.action](mess);
        } else if (mess.content.toLowerCase() == config.keyWord) {
          all.log(
            "command",
            `${mess.author.username} a utilise la commande help`
          );
          all["help"](mess);
        } else {
          mess.channel.send(
  ...
  async function updateUsers(mess: Discord.Message, sendMess = true) {
    return new Promise((resolve, reject) => {
      db.find(
        {},
        { projection: { _id: 0, id: 1, xp: 1, rank: 1, lvl: 1, avatarUrl: 1 } }
      )
        .toArray()
        .then((users) => {
          // console.log(users);
          ...
          users.forEach((user, i) => {
            ...
            db.findOneAndUpdate(
              { id: user.id },
              { $set: { rank: i + 1, avatarUrl: avatarUrl } }
            ).then(
              () => {
                config.levels.forEach((level, i) => {
                  users.forEach((user) => {
                    if (user.xp >= level.xp && i > user.lvl) {
                      ...
                      user.lvl++;
                      if (sendMess)
                        all["passeLvl"](mess, bot.users.cache.get(user.id));
                    }
                    if (user.xp < level.xp && i < user.lvl) {
                      db.findOneAndUpdate(
                        { id: user.id },
  ...
  all.startBot = async () => {
    ...
    await (bot.user as Discord.ClientUser).setActivity(
      "CostaBot v" + version + " (DM si bug)"
    );
  ...
  all.log = function (type: string) {
    ...
    if (config.interface.active) {
      try {
        WS.send(
          JSON.stringify({ action: "addLog", data: args.join(" "), type: type })
        );
      } catch (e) {
        // console.log("error to send log at interface");
      }
    }
  };
  ...
  all.help = (mess: Discord.Message) => {
    ...
    commandes.forEach((commande) => {
      if (!commande.masked && commande.access == "") {
        ...
        message.description +=
          "Utilisation: *" + config.keyWord + " " + commande.cmd + "*\n";
      }
    });
  ...
  all.send_m_xp = async (mess: Discord.Message) => {
    return new Promise((resolve, reject) => {
      db.findOne(
        { id: mess.author.id },
        { projection: { _id: 0, xp: 1, username: 1, lvl: 1, rank: 1 } }
      ).then((user) => {
        if (user == null) {
          reject(new Error("User don't exist in mongo"));
        }
        let image = new GenerateImage.XpStatus(
          mess.author.displayAvatarURL({ format: "png" }),
          user.xp,
          user.username,
          mess.author.discriminator,
          user.lvl,
          user.rank,
          () => {
            let message = new Discord.MessageAttachment(
  ...
  all.passeLvl = (mess: Discord.Message, u: Discord.User) => {
    return new Promise((resolve, reject) => {
      db.findOne(
        ...
        let image = new GenerateImage.LevelPass(
  ...
  all.send_xp_of = (mess: Discord.Message) => {
    return new Promise((resolve, reject) => {
      ...
      db.find(
        { id: { $in: mentions } },
        { projection: { _id: 0, id: 1, username: 1, xp: 1, lvl: 1, rank: 1 } }
      )
        .toArray()
        .then((result) => {
          if (result == null) {
  ...
  all.xp_reset = async (mess: Discord.Message) => {
    return new Promise((resolve, reject) => {
      ...
      db.find(
        { id: { $in: mentions } },
        { projection: { _id: 0, id: 1, username: 1, xp: 1, lvl: 1 } }
      )
        .toArray()
        .then((result) => {
          result.forEach(async (user, i) => {
            ...
            await db.findOneAndUpdate(
              { id: user.id },
              { $set: { xp: user.xp, lvl: user.lvl } }
            );
            mess.channel.send(
              `L'xp de ${user.username} a été reset (pas de chance...)`
            );
          });
          resolve(null);
        })
  ...
  all.download = async (mess: Discord.Message) => {
    if (!config.server.active) {
      mess.channel.send(
        "Erreur: Le serveur de telechargement n'est pas actif !"
      );
      return;
    }
  ...
  all.spam = async (mess: Discord.Message) => {
    let x = setInterval(() => {
      mess.channel.send(
        "ET C LE SPAM <@!" + mess.mentions.users.array()[0] + ">"
      );
    }, 1000);
    setTimeout(() => {
      clearInterval(x);
  ...
  async function addXp(id: string, createdTimestamp: number, xp?: number) {
    return new Promise((resolve, reject) => {
      db.findOne(
        { id: id },
        { projection: { _id: 0, xp: 1, lastMessage: 1 } }
      ).then((user) => {
        if (user == null) {
          reject(new Error("Utilisateur introuvable"));
          return;
        ...
        if (
          ...
          createdTimestamp - user.lastMessage > config.antispamMs
        ) {
          if (xp) {
            add = xp;
          } else {
          ...
          _xp += add;
          user.lastMessage =
            createdTimestamp != Infinity ? createdTimestamp : user.lastMessage;
        }
        db.findOneAndUpdate(
          { id: id },
          { $set: { xp: _xp, lastMessage: user.lastMessage } }
        )
          .then(() => resolve(add))
          .catch((reason) => reject(reason));
      });
  ...
  all.add_xp = async (mess: Discord.Message) => {
    ...
    let user = await db.findOne(
      { id: mention.id },
      { projection: { _id: 0, xp: 1, lvl: 1, rank: 1 } }
    );
    mess.channel.send(
      `${mention.username} vient de gagner ${
        mess.content.split(" ")[4]
      }xp et est` +
  ...
  all.set_xp = (mess: Discord.Message) => {
    let mention = mess.mentions.users.array()[0];
    db.findOne({ id: mention.id }, { projection: { _id: 0, xp: 1 } }).then(
      async (user) => {
        await db.findOneAndUpdate({ id: mention.id }, { $set: { lvl: 0 } });
        await addXp(
          mention.id,
          Infinity,
          parseInt(mess.content.split(" ")[4]) - user.xp
        );
        ...
        mess.channel.send(
          `${mention.username} a maintenant ${user.xp}xp et est maintenant level ${user.lvl} et rang ${user.rank}`
        );
      }
    );
  };
  ...
  all.classement = async (mess: Discord.Message) => {
    ...
    db.find(
      ...
      .then((users: costa_utils.user[]) => {
        new GenerateImage.Classement(
          users,
          (image: GenerateImage.Classement) => {
            let message = new Discord.MessageAttachment(
              image.toBuffer("image/png")
            );
            mess.channel.send(message);
  ...
  all.qr = (mess: Discord.Message) => {
    let m = mess.content
      .slice(mess.content.indexOf(" ") - 1, mess.content.length)
      .slice(mess.content.indexOf(" "), mess.content.length);
  ...
  all.avatar = (mess: Discord.Message) => {
    ...
    let _size = mess.content.match(/\s[0-9]+\s/) || 128;
  ...
  all.music_add = (mess: Discord.Message) => {
    ...
    if (ActualMusicPlayer.channel != mess.channel) {
      mess.channel.send(
        "Erreur: Le lecteur de musique n'est pas dans ce channel"
      );
      return;
    }
        ...
        if (member.voice)
          if (member.voice.channel)
            addXp(
              user.id,
              Infinity,
              randomInt(config.xp.voc.min, config.xp.voc.max)
            );
    });
  }, 60000);
          ...
          case "connect":
            if (!(ws as any).connected) {
              if ((ws as any).authorized(mess)) {
                ...
                ws.send(
                  JSON.stringify({ action: "connect", uptime: bot.uptime })
                );
                ws.send(
                  JSON.stringify({
                    action: "logs",
  ...
  if (config.api.active) {
    ...
    app.get("/musics/infos/all", (req, res) => {
      if (ActualMusicPlayer)
        res.status(200).json(ActualMusicPlayer.getAllInfos());
      else res.status(200).json({ Error: "No music player" });
    });
    app.get("/musics/historic/infos/all", (req, res) => {
      if (ActualMusicPlayer)
        res.status(200).json(ActualMusicPlayer.getAllHistoricInfos());
      else res.status(200).json({ Error: "No music player" });
    });
    app.get("/musics/all/infos/all", (req, res) => {
      if (ActualMusicPlayer)
        res
          .status(200)
          .json(
            ActualMusicPlayer.getAllHistoricInfos().concat(
              ActualMusicPlayer.getAllInfos() as any
            )
          );
      else res.status(200).json({ Error: "No music player" });
    });
    ...
    app.get("/musics/historic/infos", (req, res) => {
      if (ActualMusicPlayer)
        res.status(200).json(ActualMusicPlayer.getHistoricInfos());
      else res.status(200).json({ Error: "No music player" });
    });
    app.get("/musics/all/infos", (req, res) => {
      if (ActualMusicPlayer)
        res
          .status(200)
          .json(
            ActualMusicPlayer.getHistoricInfos().concat(
              ActualMusicPlayer.getInfos() as any
            )
          );
      else res.status(200).json({ Error: "No music player" });
    });
    ...
    app.post("/music/send", (req, res) => {
      let textChannel =
        req.body.textChannel || config.api.music.defaultTextChannel;
      let voiceChannel =
        req.body.voiceChannel || config.api.music.defaultVoiceChannel;
      let link = req.body.link || "";
      try {
        if (req.body.autoconnect == "true" || req.body.autoconnect == true) {
          if (ActualMusicPlayer) ActualMusicPlayer.destroy();
          ActualMusicPlayer = new MusicPlayer(
            bot.channels.cache.get(textChannel) as Discord.TextChannel,
            link,
            false
          ).on("end", () => {
            ActualMusicPlayer.send().then(() => {
              ActualMusicPlayer.connect(
                bot.channels.cache.get(voiceChannel) as Discord.VoiceChannel
              );
            });
          });
        } else {
